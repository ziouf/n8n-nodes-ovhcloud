...
import { description, execute } from './regionWorkflowBackupDeleteDelete.operation';
...
describe('region regionWorkflowBackupDeleteDelete operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct DELETE API endpoint', async () => {
			...
			expect(result).toMatchObject([{ deleted: 'test-backup-id' }]);
		});
	});
});
