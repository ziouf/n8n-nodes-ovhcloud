 ...
 *
 * @remarks
 * Commercial catalog is managed under `/v2/commercialCatalog/offers` route.
...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	...
	return [
		...
		...descriptionList({
			...displayOptions,
			show: { ...displayOptions?.show, commercialCatalogOperation: ['list'] },
		}),
		...descriptionGet({
			...displayOptions,
			show: { ...displayOptions?.show, commercialCatalogOperation: ['get'] },
		}),
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const operation = this.getNodeParameter('commercialCatalogOperation', 0, {
		extractValue: true,
	});
	...
	throw new Error(`Unsupported operation "${operation}" for resource "commercialCatalog"`);
}
