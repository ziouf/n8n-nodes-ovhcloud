...
export class OvhCloudCommercialCatalog extends BaseNode implements INodeType {
	description: INodeTypeDescription = {
		...
		defaults: {
			name: 'OVH Cloud Commercial Catalog',
		},
		usableAsTool: true,
		...
		return executeTemplate.call(this, execute, {
			perItemConcurrency: {
				classify: (ctx, itemIndex) =>
					classifyOperation(
						String(ctx.getNodeParameter('commercialCatalogOperation', itemIndex, { extractValue: true })),
					),
			},
			errorContext: { resource: 'commercialcatalog', operationParam: 'commercialCatalogOperation' },
		});
	}
}
