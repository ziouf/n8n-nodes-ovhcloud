...
import { description, execute } from './serviceConfirmTerminationPost.operation';
...
describe('officePrepaid {serviceName}ConfirmTerminationPOST operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint', async () => {
			...
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string, _idx?: number, def?: any, opts?: any): any => {
				...
				if (param === 'commentary') return 'test-commentary';
				if (param === 'futureUse') return 'test-futureUse';
				if (param === 'reason') return 'test-reason';
				return def ?? '';
			});
