...
import { description, execute } from './tenantUsageStatisticsGet.operation';
...
describe('officePrepaid {serviceName}TenantUsageStatisticsGET operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint', async () => {
			...
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string, _idx?: number, def?: any, opts?: any): any => {
				...
				if (param === 'from') return 'test-from';
				if (param === 'to') return 'test-to';
				return def ?? '';
			});
