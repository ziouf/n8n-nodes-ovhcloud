...
import { description, execute } from './usersListGet.operation';
...
describe('office {serviceName}UserGET operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint', async () => {
			...
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string, _idx?: number, def?: any, opts?: any): any => {
				...
				if (param === 'activationEmail') return 'test-activationEmail';
				if (param === 'firstName') return 'test-firstName';
				if (param === 'lastName') return 'test-lastName';
				if (param === 'licences') return 'test-licences';
				return def ?? '';
			});
