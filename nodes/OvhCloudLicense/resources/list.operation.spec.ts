...
describe('list.operation', () => {
	...
	describe('execute', () => {
		...
		it('should list WorkLight licenses via GET', async () => {
			const mockData = ['license-1', 'license-2'];
			const client = new ApiClient(mockExecuteFunctions) as any;
			client.httpGet.mockResolvedValue(mockData);

			const result = await execute.call(mockExecuteFunctions);
			expect(client.httpGet).toHaveBeenCalledWith('/license/worklight');
			expect(result).toEqual([{ serviceName: 'license-1' }, { serviceName: 'license-2' }]);
		});
