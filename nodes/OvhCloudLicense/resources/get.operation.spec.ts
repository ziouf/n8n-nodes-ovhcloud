...
describe('get.operation', () => {
	describe('description', () => {
		...
		it('should have list and name modes for the service locator', () => {
			...
			expect(nameMode.placeholder).toBe('license-1');
		});
	});
	...
	describe('execute', () => {
		...
		it('should get WorkLight license properties via GET', async () => {
			const mockData = { domain: 'license-1', ip: '123.45.67.89', status: 'ok' };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/license/worklight/license-1');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/license/worklight/license%201');
		});
	});
});
