...
import { description, execute } from './sqlServerPost.operation';
...
describe('windows {serviceName}SqlServerPOST operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint', async () => {
			...
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string, _idx?: number, def?: any, opts?: any): any => {
				...
				if (param === 'licenseId') return 'test-licenseId';
				if (param === 'version') return 'test-version';
				return def ?? '';
			});
