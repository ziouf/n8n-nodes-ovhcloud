...
| [`list`](./`resources/list.ts`)                                                   | GET    | `...`                                                                    | 0     | (Includes "Return Full Objects / Max Items" toggle for parallel fetching) |
...
- **List operation** — supports a "Return Full Objects / Max Items" toggle: when enabled, full server objects are fetched in parallel with a configurable max item count. A warning item is prepended to results if some resources could not be fetched.
- **Destructive operations** — operations like `confirmTerminationCreate` display a yellow warning notice in the node UI to alert users before irreversible actions.
