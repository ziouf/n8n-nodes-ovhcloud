/**
 * @brief Delete a pool from a Dedicated Ceph cluster
 *
 * Deletes the specified pool:
 * - HTTP DELETE request to `/dedicated/ceph/{serviceName}/pool/{poolName}` endpoint
 */
import type {
	IDisplayOptions,
	IExecuteFunctions,
...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'The pool name to delete',
			displayOptions,
		},
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	await client.httpDelete(`/dedicated/ceph/${serviceName}/pool/${poolName}`);
