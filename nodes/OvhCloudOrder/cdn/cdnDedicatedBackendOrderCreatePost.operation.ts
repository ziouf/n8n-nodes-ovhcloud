...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	...
	const backend = this.getNodeParameter('backend', itemIndex) as number;

	const body = {backend: backend};

	const data = (await client.httpPost(`/order/cdn/dedicated/${serviceName}/backend/${duration}`, body)) as IDataObject;
