...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpDelete("/email/exchange/" + encodeURIComponent(organizationName) + "/service/" + encodeURIComponent(exchangeService) + "/publicFolder/" + encodeURIComponent(path) + "/permission/" + encodeURIComponent(allowedAccountId))) as IDataObject;
