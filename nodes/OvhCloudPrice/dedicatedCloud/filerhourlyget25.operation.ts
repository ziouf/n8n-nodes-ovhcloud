 ...
 *
 * HTTP method: GET
 * Endpoint: /price/dedicatedCloud/2016v1/sbg1a/enterprise/filer/hourly/{filerProfile}
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
    const client = new ApiClient(this);
    const filerProfile = this.getNodeParameter('filerProfile', _itemIndex ?? 0) as string;
    const data = (await client.httpGet(`/price/dedicatedCloud/2016v1/sbg1a/enterprise/filer/hourly/${filerProfile}`)) as IDataObject;
