 ...
 *
 * HTTP method: GET
 * Endpoint: /price/dedicatedCloud/2016v5/sbg1a/enterprise/host/monthly/{hostProfile}
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
    const client = new ApiClient(this);
    const hostProfile = this.getNodeParameter('hostProfile', _itemIndex ?? 0) as string;
    const data = (await client.httpGet(`/price/dedicatedCloud/2016v5/sbg1a/enterprise/host/monthly/${hostProfile}`)) as IDataObject;
