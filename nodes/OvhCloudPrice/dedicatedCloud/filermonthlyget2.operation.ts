 ...
 *
 * HTTP method: GET
 * Endpoint: /price/dedicatedCloud/2013v1/sbg1a/filer/monthly/{filerProfile}
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
    const client = new ApiClient(this);
    const filerProfile = this.getNodeParameter('filerProfile', 0) as string;
    const data = (await client.httpGet(`/price/dedicatedCloud/2013v1/sbg1a/filer/monthly/${filerProfile}`)) as IDataObject;
