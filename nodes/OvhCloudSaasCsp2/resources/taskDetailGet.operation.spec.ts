...
describe('taskDetailGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get a task via GET', async () => {
			const mockData = { id: 55, status: 'done' };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/saas/csp2/csp2-12345/task/55');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/saas/csp2/csp%202/task/55');
		});
	});
});
