...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: 0,
			required: true,
			description: 'The identifier of the orderable Office license',
			displayOptions,
		},
	];
}
...
/**
 * Get properties of an orderable Office license.
 *
 * HTTP method: GET
 * Endpoint: /saas/csp2/{serviceName}/orderableLicenses/{id}
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(
		`/saas/csp2/${encodeURIComponent(serviceName)}/orderableLicenses/${encodeURIComponent(id)}`,
	)) as IDataObject;
