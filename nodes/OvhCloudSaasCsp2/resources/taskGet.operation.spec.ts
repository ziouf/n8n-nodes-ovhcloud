...
describe('taskGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should list running tasks via GET', async () => {
			const mockData = [55, 56];
			...
			expect(client.httpGet).toHaveBeenCalledWith('/saas/csp2/csp2-12345/task');
			expect(result).toEqual([{ taskId: 55 }, { taskId: 56 }]);
		});

		it('should encode special characters in serviceName', async () => {
			const client = new ApiClient(mockExecuteFunctions) as any;
			client.httpGet.mockResolvedValue([55]);
			...
			expect(client.httpGet).toHaveBeenCalledWith('/saas/csp2/csp%202/task');
		});
	});
});
