...
/**
 * Create a log subscription for an Email Exchange service.
 *
 * HTTP method: POST
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const kindName = this.getNodeParameter('kindName', 0) as string;
	const displayName = this.getNodeParameter('displayName', 0) as string;

	const body: IDataObject = { kindName, displayName };
