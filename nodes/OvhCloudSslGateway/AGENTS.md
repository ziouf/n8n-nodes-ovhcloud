...
- **API Spec**: https://eu.api.ovh.com/v1/sslGateway.json
- **Base Path**: /sslGateway
...
├── list.operation.ts      # GET /sslGateway
├── get.operation.ts       # GET /sslGateway/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getSslGatewayServices'`)
