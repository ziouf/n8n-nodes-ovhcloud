...
import { description, execute } from './deleteUser.operation';
...
describe('user/deleteUser.operation', () => {
	describe('description', () => {
		...
		it('should include displayOptions in the parameter when provided', () => {
			const result = description({ show: { webResourceOperation: ['deleteUser'] } });
			expect((result[0] as any).displayOptions.show.webResourceOperation).toEqual(['deleteUser']);
		});
	});
	...
	describe('execute', () => {
		...
		it('should delete user via DELETE', async () => {
