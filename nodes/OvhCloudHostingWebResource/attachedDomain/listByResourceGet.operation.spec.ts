...
describe('attachedDomain/listByResourceGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should list attached domains via GET', async () => {
		...
		it('should return attached domains wrapped in array', async () => {
			const mockData = [{ name: 'example.com' }];
			(new ApiClient(mockExecuteFunctions) as any).httpGet.mockResolvedValue(mockData);
			mockExecuteFunctions.getNodeParameter.mockReturnValue('myResource');
