...
import { description, execute } from './deleteDeleteByResourceNameGet.operation';
...
describe('attachedDomain/deleteDeleteByResourceNameGet.operation', () => {
	describe('description', () => {
		...
		it('should include displayOptions in the parameter when provided', () => {
			const result = description({ show: { webResourceOperation: ['getByNameGet'] } });
			expect((result[0] as any).displayOptions.show.webResourceOperation).toEqual(['getByNameGet']);
		});
	});
