...
- **API Spec**: https://eu.api.ovh.com/v1/email/domain.json
- **Base Path**: /email/domain
...
├── list.operation.ts      # GET /email/domain
├── get.operation.ts       # GET /email/domain/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getEmailServices'`)
