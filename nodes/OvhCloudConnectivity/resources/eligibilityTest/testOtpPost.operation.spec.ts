...
import { description, execute } from './testOtpPost.operation';
...
describe('testOtpPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/connectivity/eligibility/test/otp', { otp: 'sample-value' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/connectivity/eligibility/test/otp', { otp: 'alternate-value' });
		});
	});
});