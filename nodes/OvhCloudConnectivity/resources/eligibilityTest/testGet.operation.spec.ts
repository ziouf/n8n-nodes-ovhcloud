...
import { description, execute } from './testGet.operation';
...
describe('testGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/connectivity/eligibility/test', { eligibilityReference: 'sample-value' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/connectivity/eligibility/test', { eligibilityReference: 'alternate-value' });
		});
	});
});