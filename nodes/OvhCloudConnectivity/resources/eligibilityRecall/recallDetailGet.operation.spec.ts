...
import { description, execute } from './recallDetailGet.operation';
...
describe('recallDetailGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/connectivity/eligibility/recall/12345');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/connectivity/eligibility/recall/98765');
		});
	});
});