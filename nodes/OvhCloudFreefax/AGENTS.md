...
- **API Spec**: https://eu.api.ovh.com/v1/freefax.json
- **Base Path**: /freefax
...
├── list.operation.ts      # GET /freefax
├── get.operation.ts       # GET /freefax/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getFreefaxServices'`)
