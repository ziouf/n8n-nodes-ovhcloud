...
describe('serviceInfosGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get service information via GET', async () => {
			const mockData = { domain: 'example.com', state: 'ok' };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/freefax/fr12345-ovh/serviceInfos');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/freefax/fr%2012345-ovh/serviceInfos');
		});
	});
});
