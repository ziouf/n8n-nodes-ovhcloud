...
import { description, execute } from './directoryGet.operation';
...
describe('directoryGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get directory information via GET', async () => {
			const mockData = { name: 'Example Name', city: 'Paris' };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/freefax/fr12345-ovh/directory');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/freefax/fr%2012345-ovh/directory');
		});
	});
});
