...
import { description, execute } from './voicemailGet.operation';
...
describe('voicemailGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get voicemail properties via GET', async () => {
			const mockData = { audioFormat: 'mp3', isNewVersion: true };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/freefax/fr12345-ovh/voicemail');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/freefax/fr%2012345-ovh/voicemail');
		});
	});
});
