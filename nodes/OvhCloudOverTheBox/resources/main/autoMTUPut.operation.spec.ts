...
import { description, execute } from './autoMTUPut.operation';
...
describe('autoMTUPut.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via PUT', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/overTheBox/svc-1/autoMTU', {
				mtuAuto: 'mtuAuto-v',
			});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/overTheBox/svc%201/autoMTU', {});
		});
	});
});
