...
import { description, execute } from './ipsGet.operation';
...
describe('ipsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/ips');

			expect(result).toEqual([{ ip: 'id-1' }, { ip: 'id-2' }]);
		});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/ips');
		});
	});
});
