...
import { description, execute } from './ipv6Put.operation';
...
describe('ipv6Put.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via PUT', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/overTheBox/svc-1/ipv6', { enabled: true });
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/overTheBox/svc%201/ipv6', {});
		});
	});
});
