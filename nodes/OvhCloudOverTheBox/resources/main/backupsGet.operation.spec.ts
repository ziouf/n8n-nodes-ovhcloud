...
import { description, execute } from './backupsGet.operation';
...
describe('backupsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/backups');

			expect(result).toEqual([{ backupId: 'id-1' }, { backupId: 'id-2' }]);
		});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/backups');
		});
	});
});
