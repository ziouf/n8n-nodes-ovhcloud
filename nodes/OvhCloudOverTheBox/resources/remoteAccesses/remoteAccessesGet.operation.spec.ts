...
import { description, execute } from './remoteAccessesGet.operation';
...
describe('remoteAccessesGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/remoteAccesses');

			expect(result).toEqual([{ remoteAccessId: 'id-1' }, { remoteAccessId: 'id-2' }]);
		});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/remoteAccesses');
		});
	});
});
