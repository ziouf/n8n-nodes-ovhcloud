...
import { description, execute } from './remoteAccessDelete.operation';
...
describe('remoteAccessDelete.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via DELETE', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith(
				'/overTheBox/svc-1/remoteAccesses/remoteAccessId-v',
			);
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith(
				'/overTheBox/svc%201/remoteAccesses/remoteAccessId-v',
			);
		});
	});
});
