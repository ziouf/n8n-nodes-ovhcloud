...
import { description, execute } from './remoteAccessAuthorizePost.operation';
...
describe('remoteAccessAuthorizePost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/overTheBox/svc-1/remoteAccesses/remoteAccessId-v/authorize',
			);
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/overTheBox/svc%201/remoteAccesses/remoteAccessId-v/authorize',
			);
		});
	});
});
