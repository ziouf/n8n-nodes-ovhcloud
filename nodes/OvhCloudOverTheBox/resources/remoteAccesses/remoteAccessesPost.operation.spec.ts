...
import { description, execute } from './remoteAccessesPost.operation';
...
describe('remoteAccessesPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc-1/remoteAccesses', {
				allowedIp: 'allowedIp-v',
				expirationDate: 'expirationDate-v',
				exposedPort: 1,
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc%201/remoteAccesses', {});
		});
	});
});
