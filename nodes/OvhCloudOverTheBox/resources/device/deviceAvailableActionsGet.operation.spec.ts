...
import { description, execute } from './deviceAvailableActionsGet.operation';
...
describe('deviceAvailableActionsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/device/availableActions');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/device/availableActions');
		});
	});
});
