...
import { description, execute } from './deviceActionsGet.operation';
...
describe('deviceActionsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/device/actions', {
				name: 'name-v',
				status: 'status-v',
			});

			expect(result).toEqual([{ actionId: 'id-1' }, { actionId: 'id-2' }]);
		});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/device/actions', {});
		});
	});
});
