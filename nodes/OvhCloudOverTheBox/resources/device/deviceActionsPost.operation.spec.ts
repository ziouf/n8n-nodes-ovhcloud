...
import { description, execute } from './deviceActionsPost.operation';
...
describe('deviceActionsPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc-1/device/actions', {
				name: 'name-v',
			});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc%201/device/actions', {});
		});
	});
});
