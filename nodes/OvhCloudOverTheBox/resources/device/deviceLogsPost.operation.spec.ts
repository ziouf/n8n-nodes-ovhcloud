...
import { description, execute } from './deviceLogsPost.operation';
...
describe('deviceLogsPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc-1/device/logs');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc%201/device/logs');
		});
	});
});
