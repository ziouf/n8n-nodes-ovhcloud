...
import { description, execute } from './deviceActionGet.operation';
...
describe('deviceActionGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/device/actions/actionId-v');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/device/actions/actionId-v');
		});
	});
});
