...
import { description, execute } from './deviceBackupPost.operation';
...
describe('deviceBackupPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc-1/device/backup');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc%201/device/backup');
		});
	});
});
