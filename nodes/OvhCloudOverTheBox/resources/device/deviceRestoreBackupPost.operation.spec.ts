...
import { description, execute } from './deviceRestoreBackupPost.operation';
...
describe('deviceRestoreBackupPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc-1/device/restoreBackup', {
				backupId: 'backupId-v',
			});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/overTheBox/svc%201/device/restoreBackup', {});
		});
	});
});
