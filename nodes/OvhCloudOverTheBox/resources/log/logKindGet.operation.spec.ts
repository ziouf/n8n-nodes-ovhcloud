...
describe('logKindGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/log/kind');

			expect(result).toEqual([{ kind: 'id-1' }, { kind: 'id-2' }]);
		});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/log/kind');
		});
	});
});
