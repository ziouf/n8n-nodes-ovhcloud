...
import { description, execute } from './logSubscriptionGet.operation';
...
describe('logSubscriptionGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc-1/log/subscription', {
				kind: 'kind-v',
			});

			expect(result).toEqual([{ subscriptionId: 'id-1' }, { subscriptionId: 'id-2' }]);
		});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/overTheBox/svc%201/log/subscription', {});
		});
	});
});
