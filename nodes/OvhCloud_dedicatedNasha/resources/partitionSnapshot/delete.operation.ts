...
/**
 * @brief Delete Partition Snapshot operation for DedicatedNasha resource
 *
 * Deletes a specific scheduled snapshot:
 * - HTTP DELETE request to `/dedicated/nasha/{serviceName}/partition/{partitionName}/snapshot/{snapshotType}` endpoint
 * - Service name, partition name, and snapshot type are required
 * - Returns the deletion result
 */
export function descriptionDedicatedNashaPartitionSnapshotDelete(
	...
	return [
		...
		},
		{
			...
			name: 'snapshotType',
			description: 'The type of the scheduled snapshot to delete',
			type: 'string',
			required: true,
			default: '',
			displayOptions,
...
/**
 * Executes the Delete Partition Snapshot operation.
 *
 * Deletes a specific scheduled snapshot.
 *
 * HTTP method: DELETE
 ...
 */
export async function executeDedicatedNashaPartitionSnapshotDelete(
