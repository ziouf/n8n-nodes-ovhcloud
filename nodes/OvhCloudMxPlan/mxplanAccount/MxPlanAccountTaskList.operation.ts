	...
	return [
		{
			...
			displayOptions: {
				show: {
					mxPlanOperation: ['MxPlanAccountTaskList'],
				},
				...displayOptions,
			},
		},
 ...
 *
 * HTTP method: GET
 * Endpoint: /email/mxplan/{service}/account/{email}/task
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet('/email' + '/mxplan/' + encodeURIComponent(service) + '/account/' + encodeURIComponent(email) + '/task')) as IDataObject;
