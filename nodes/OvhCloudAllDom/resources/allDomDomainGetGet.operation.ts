...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'The domain name attached to this AllDom service',
			displayOptions,
		},
	];
}
...
/**
 * Get properties of an AllDom domain.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(
		`/allDom/${encodeURIComponent(serviceName)}/domain/${encodeURIComponent(domain)}`,
	)) as IDataObject;
