...
import { description, execute } from './networkGetGet.operation';
...
describe('networkGetGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct endpoint', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				if (param === 'networkId') return 'network-1';
			...
			expect(client.httpGet).toHaveBeenCalledWith('/storage/netapp/pcc-123-456-789/network/network-1');
		});
	});
});
