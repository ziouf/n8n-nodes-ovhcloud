...
import { description, execute } from './snapshotPolicyCreatePost.operation';
...
describe('snapshotPolicyCreatePost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct endpoint', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				...
				if (param === 'isDefault') return true;
				if (param === 'rules') return 'value';
				return undefined;
			});
			...
			expect(client.httpPost).toHaveBeenCalledWith('/storage/netapp/pcc-123-456-789/snapshotPolicy', { description: "value", name: "value", isDefault: true, rules: "value" });
		});
	});
});
