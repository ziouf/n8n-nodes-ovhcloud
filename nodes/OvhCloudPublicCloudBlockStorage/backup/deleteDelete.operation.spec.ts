...
describe('backup deleteDelete.operation', () => {
	...
	describe('execute', () => {
		...
		it('should delete backup via DELETE', async () => {
			...
			expect(result).toEqual([{ deleted: 'bkp-id' }]);
		});

		it('should return the backup id in result when deletion succeeds', async () => {
			const client = new ApiClient(mockExecuteFunctions) as any;
			client.httpDelete.mockResolvedValue({ ok: true });
			...
			const result = await execute.call(mockExecuteFunctions);
			expect(result[0].deleted).toBe('test-bkp-id');
		});
	});
});
