...
/**
 * Executes the Create a contact operation.
 *
 * HTTP method: POST
...
export async function execute(this: IExecuteFunctions, _itemIndex: number): Promise<INodeExecutionData[]> {
	...
	const body: IDataObject = {};
		const accreditationCountry = this.getNodeParameter('accreditationCountry', _itemIndex, '') as string;
		if (accreditationCountry !== '') body['accreditationCountry'] = accreditationCountry;
		const accreditationId = this.getNodeParameter('accreditationId', _itemIndex, '') as string;
		if (accreditationId !== '') body['accreditationId'] = accreditationId;
		const accreditationOrganism = this.getNodeParameter('accreditationOrganism', _itemIndex, '') as string;
		if (accreditationOrganism !== '') body['accreditationOrganism'] = accreditationOrganism;
		const accreditationYear = this.getNodeParameter('accreditationYear', _itemIndex, 0) as number;
		...
		const birthCity = this.getNodeParameter('birthCity', _itemIndex, '') as string;
		if (birthCity !== '') body['birthCity'] = birthCity;
		const birthCountry = this.getNodeParameter('birthCountry', _itemIndex, '') as string;
		if (birthCountry !== '') body['birthCountry'] = birthCountry;
		const birthDay = this.getNodeParameter('birthDay', _itemIndex, '') as string;
		if (birthDay !== '') body['birthDay'] = birthDay;
		const birthZip = this.getNodeParameter('birthZip', _itemIndex, '') as string;
		if (birthZip !== '') body['birthZip'] = birthZip;
		const cellPhone = this.getNodeParameter('cellPhone', _itemIndex, '') as string;
		if (cellPhone !== '') body['cellPhone'] = cellPhone;
		const companyNationalIdentificationNumber = this.getNodeParameter('companyNationalIdentificationNumber', _itemIndex, '') as string;
		...
		const enterpriseId = this.getNodeParameter('enterpriseId', _itemIndex, '') as string;
		if (enterpriseId !== '') body['enterpriseId'] = enterpriseId;
		const fax = this.getNodeParameter('fax', _itemIndex, '') as string;
		if (fax !== '') body['fax'] = fax;
		...
		const insee = this.getNodeParameter('insee', _itemIndex, '') as string;
		...
		const legalFormCategory = this.getNodeParameter('legalFormCategory', _itemIndex, '') as string;
		if (legalFormCategory !== '') body['legalFormCategory'] = legalFormCategory;
		const nationalIdentificationNumber = this.getNodeParameter('nationalIdentificationNumber', _itemIndex, '') as string;
		if (nationalIdentificationNumber !== '') body['nationalIdentificationNumber'] = nationalIdentificationNumber;
		const nationality = this.getNodeParameter('nationality', _itemIndex, '') as string;
		if (nationality !== '') body['nationality'] = nationality;
		const organisationAccountable = this.getNodeParameter('organisationAccountable', _itemIndex, '') as string;
		if (organisationAccountable !== '') body['organisationAccountable'] = organisationAccountable;
		const organisationFunding = this.getNodeParameter('organisationFunding', _itemIndex, '') as string;
		if (organisationFunding !== '') body['organisationFunding'] = organisationFunding;
		const organisationFundingOther = this.getNodeParameter('organisationFundingOther', _itemIndex, '') as string;
		if (organisationFundingOther !== '') body['organisationFundingOther'] = organisationFundingOther;
		const organisationName = this.getNodeParameter('organisationName', _itemIndex, '') as string;
		if (organisationName !== '') body['organisationName'] = organisationName;
		const organisationRole = this.getNodeParameter('organisationRole', _itemIndex, '') as string;
		if (organisationRole !== '') body['organisationRole'] = organisationRole;
		const organisationRoleOther = this.getNodeParameter('organisationRoleOther', _itemIndex, '') as string;
		if (organisationRoleOther !== '') body['organisationRoleOther'] = organisationRoleOther;
		const organisationStaffStatus = this.getNodeParameter('organisationStaffStatus', _itemIndex, '') as string;
		if (organisationStaffStatus !== '') body['organisationStaffStatus'] = organisationStaffStatus;
		const organisationStaffStatusOther = this.getNodeParameter('organisationStaffStatusOther', _itemIndex, '') as string;
		if (organisationStaffStatusOther !== '') body['organisationStaffStatusOther'] = organisationStaffStatusOther;
		const organisationType = this.getNodeParameter('organisationType', _itemIndex, '') as string;
		if (organisationType !== '') body['organisationType'] = organisationType;
		const organisationTypeOther = this.getNodeParameter('organisationTypeOther', _itemIndex, '') as string;
		if (organisationTypeOther !== '') body['organisationTypeOther'] = organisationTypeOther;
		const phone = this.getNodeParameter('phone', _itemIndex, '') as string;
		if (phone !== '') body['phone'] = phone;
		const registrantDocumentType = this.getNodeParameter('registrantDocumentType', _itemIndex, '') as string;
		if (registrantDocumentType !== '') body['registrantDocumentType'] = registrantDocumentType;
		const registrantDocumentTypeOther = this.getNodeParameter('registrantDocumentTypeOther', _itemIndex, '') as string;
		if (registrantDocumentTypeOther !== '') body['registrantDocumentTypeOther'] = registrantDocumentTypeOther;
		const roleInOrganisation = this.getNodeParameter('roleInOrganisation', _itemIndex, '') as string;
		if (roleInOrganisation !== '') body['roleInOrganisation'] = roleInOrganisation;
		const trademarkId = this.getNodeParameter('trademarkId', _itemIndex, '') as string;
		if (trademarkId !== '') body['trademarkId'] = trademarkId;
		const vat = this.getNodeParameter('vat', _itemIndex, '') as string;
		if (vat !== '') body['vat'] = vat;
		const website = this.getNodeParameter('website', _itemIndex, '') as string;
