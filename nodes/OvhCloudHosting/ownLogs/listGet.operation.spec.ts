...
describe('listGet.operation', () => {
		describe('description', () => {
			it('should return the expected parameters', () => {
				...
				expect(result.map((p) => p.name)).toEqual(expect.arrayContaining(["serviceName", "fqdn"]));
			});
		});

		describe('execute', () => {
			it('should call GET /hosting/web/{serviceName}/ownLogs', async () => {
				const mockExecuteFunctions: any = {
					getNodeParameter: jest.fn(),
					helpers: { returnJsonArray: jest.fn((data) => data) },
				};
				mockExecuteFunctions.getNodeParameter.mockImplementation((p: string): any =>
						p === 'serviceName' ? 'myservice.ovh' : p === 'fqdn' ? '' : 'default-fallback',
					);
				await execute.call(mockExecuteFunctions, 0);
				expect(ApiClient).toHaveBeenCalled();
				const client = new ApiClient(mockExecuteFunctions) as any;
			expect(client.httpGet).toHaveBeenCalledWith(`/hosting/web/myservice.ovh/ownLogs`, {});
			});
		});
});
