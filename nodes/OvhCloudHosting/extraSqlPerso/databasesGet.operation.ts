...
/**
 * Get the databases linked to an extra SQL perso option
 *
 * HTTP method: GET
...
export async function execute(
	...
	const data = (await client.httpGet(
		`/hosting/web/${encodeURIComponent(serviceName)}/extraSqlPerso/${encodeURIComponent(String(id))}/databases`,
	)) as string[];
	return this.helpers.returnJsonArray(data.map((database) => ({ database })));
}
