...
describe('updatePut.operation', () => {
		describe('description', () => {
			it('should return the expected parameters', () => {
				...
				expect(result.map((p) => p.name)).toEqual(expect.arrayContaining(["serviceName", "email"]));
			});
		});

		describe('execute', () => {
			it('should call PUT /hosting/web/{serviceName}/email', async () => {
				const mockExecuteFunctions: any = {
					getNodeParameter: jest.fn(),
					helpers: { returnJsonArray: jest.fn((data) => data) },
				};
				mockExecuteFunctions.getNodeParameter.mockImplementation((p: string): any =>
						p === 'serviceName' ? 'myservice.ovh' : p === 'email' ? 'admin@myservice.ovh' : 'default-fallback',
					);
				await execute.call(mockExecuteFunctions, 0);
				expect(getClient).toHaveBeenCalled();
				const client = new ApiClient(mockExecuteFunctions) as any;
			expect(client.httpPut).toHaveBeenCalledWith(`/hosting/web/myservice.ovh/email`, {email: 'admin@myservice.ovh'});
			});
		});
});
