...
export async function executeServiceInfosUpdate(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const client = new ApiClient(this);
	const serviceName = this.getNodeParameter('serviceName', 0, { extractValue: true }) as string;

	const response = (await client.httpPut(`/vps/${serviceName}/serviceInfos`)) as IDataObject;
