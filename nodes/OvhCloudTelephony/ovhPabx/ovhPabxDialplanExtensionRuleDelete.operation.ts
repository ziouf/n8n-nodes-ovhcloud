...
/**
 * Executes the OvhPabxDialplanExtensionRuleDelete operation.
 *
 * HTTP method: DELETE
...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpDelete('/telephony/' + encodeURIComponent(billingAccount) + '/ovhPabx' + '/' + encodeURIComponent(serviceName) + '/dialplan' + '/' + encodeURIComponent(dialplanId) + '/extension' + '/' + encodeURIComponent(extensionId) + '/rule' + '/' + encodeURIComponent(ruleId))) as IDataObject;
