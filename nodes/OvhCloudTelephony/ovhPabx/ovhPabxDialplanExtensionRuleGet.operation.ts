...
/**
 * Executes the OvhPabxDialplanExtensionRuleGet operation.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions, _itemIndex: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet('/telephony/' + encodeURIComponent(billingAccount) + '/ovhPabx' + '/' + encodeURIComponent(serviceName) + '/dialplan' + '/' + encodeURIComponent(dialplanId) + '/extension' + '/' + encodeURIComponent(extensionId) + '/rule' + '/' + encodeURIComponent(ruleId))) as IDataObject;
