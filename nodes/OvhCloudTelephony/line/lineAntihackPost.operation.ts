import type {
	IDataObject,
	IExecuteFunctions,
	IDisplayOptions,
	INodeExecutionData,
	INodeProperties,
} from 'n8n-workflow';
import { ApiClient } from '../../../shared/transport/ApiClient';

export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
        {
          displayName: 'Billing Account',
          name: 'billingAccount',
          type: 'string',
          default: '',
          required: true,
          description: 'The name of your billingAccount',
          displayOptions,
        },
        {
          displayName: 'Service Name',
          name: 'serviceName',
          type: 'string',
          default: '',
          required: true,
          displayOptions,
        },
        {
          displayName: 'Action',
          name: 'action',
          type: 'string',
          default: '',
          required: true,
          description: 'The action parameter',
          displayOptions,
        },
        {
          displayName: 'Restricted',
          name: 'restricted',
          type: 'string',
          default: '',
          description: 'The restricted parameter',
          displayOptions,
        },
	];
}

/**
 * Executes the Post Line Antihack Create operation.
 *
 * HTTP method: POST
 * Endpoint: /telephony/{billingAccount}/line/{serviceName}/antihack
 */
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	const billingAccount = this.getNodeParameter('billingAccount', itemIndex) as string;
	const serviceName = this.getNodeParameter('serviceName', itemIndex) as string;
	const action = this.getNodeParameter('action', itemIndex) as string;
	const restricted = this.getNodeParameter('restricted', itemIndex) as string;

	const body: IDataObject = {
    action: action,
    restricted: restricted
    };

	const client = new ApiClient(this);
	const data = (await client.httpPost('/telephony/' + billingAccount + '/line/' + serviceName + '/antihack', body)) as IDataObject;
	return this.helpers.returnJsonArray([data]);
}
