...
- **API Spec**: https://eu.api.ovh.com/v1/telephony.json
- **Base Path**: /telephony
...
├── list.operation.ts      # GET /telephony
├── get.operation.ts       # GET /telephony/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getTelephonyServices'`)
