...
/**
 * Executes the List backup repositories operation.
 *
 * HTTP method: GET
 * Endpoint: /dedicatedCloud/{serviceName}/backupRepository
 */
export async function execute(this: IExecuteFunctions, _itemIndex: number): Promise<INodeExecutionData[]> {
	const client = new ApiClient(this);
	const serviceName = this.getNodeParameter('serviceName', _itemIndex) as string;
	const data = (await client.httpGet(`/dedicatedCloud/${serviceName}/backupRepository`)) as IDataObject;
