...
import { description, execute } from './resizeNsxtEdgeCluster.operation';
...
describe('resizeNsxtEdgeCluster.operation', () => {
	...
	describe('execute', () => {
		...
		it('should POST to the correct endpoint with the size body', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				if (param === 'datacenterId') return 1;
				if (param === 'serviceName') return 'pcc-123-456-789';
				if (param === 'size') return 'LARGE';
				return undefined;
			});
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/dedicatedCloud/pcc-123-456-789/datacenter/1/resizeNsxtEdgeCluster',
				{ size: 'LARGE' },
			);
		});
	});
