...
import { description, execute } from './zertoUsageReport.operation';
...
describe('zertoUsageReport.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct endpoint', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				...
				if (param === 'month') return 1;
				if (param === 'year') return 1;
			...
			expect(client.httpGet).toHaveBeenCalledWith('/dedicatedCloud/pcc-123-456-789/datacenter/1/disasterRecovery/zerto/usageReport', { month: 1, year: 1 });
		});
	});
});
