...
import { description, execute } from './stockZpoolList.operation';
...
describe('stockZpoolList.operation', () => {
	...
	describe('execute', () => {
		...
		it('should list datastores stock via GET and respect limit', async () => {
			const mockData = [{ name: 'stock-01' }, { name: 'stock-02' }];
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				if (param === 'pccZone') return 'eu-west-1a';
				if (param === 'profileFilter') return '';
			...
			expect(client.httpGet).toHaveBeenCalledWith(
				'/dedicatedCloud/location/eu-west-1a/stock/zpool',
				{},
			);
