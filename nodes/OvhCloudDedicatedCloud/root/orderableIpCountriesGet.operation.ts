...
/**
 * Executes the Get Orderable IP Block Countries operation.
 *
 * HTTP method: GET
 * Endpoint: /dedicatedCloud/{serviceName}/orderableIpCountries
 */
export async function execute(this: IExecuteFunctions, _itemIndex: number): Promise<INodeExecutionData[]> {
	const client = getClient(this);
	const serviceName = this.getNodeParameter('serviceName', _itemIndex) as string;
	const data = (await client.httpGet(`/dedicatedCloud/${serviceName}/orderableIpCountries`)) as IDataObject;
