...
import { description, execute } from './vmDisableCarp.operation';
...
describe('vmDisableCarp.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct endpoint', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				...
				if (param === 'macAddress') return 'value';
				return undefined;
			});
			...
			expect(client.httpPost).toHaveBeenCalledWith('/dedicatedCloud/pcc-123-456-789/datacenter/1/vm/1/disableCarp', { macAddress: "value" });
		});
	});
});
