...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	...
	body.filerId = this.getNodeParameter('filerId', itemIndex) as number;
	const filerType = this.getNodeParameter('filerType', itemIndex, '') as string;
	if (filerType !== '') { body.filerType = filerType; }
	body.restorePointId = this.getNodeParameter('restorePointId', itemIndex) as number;
