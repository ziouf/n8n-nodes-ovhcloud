...
import { description, execute } from './vmSetLicense.operation';
...
describe('vmSetLicense.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct endpoint', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				...
				if (param === 'bypassGuestOsFamilyCheck') return true;
				if (param === 'kmsLicense') return 'value';
				return undefined;
			});
			...
			expect(client.httpPost).toHaveBeenCalledWith('/dedicatedCloud/pcc-123-456-789/datacenter/1/vm/1/setLicense', { bypassGuestOsFamilyCheck: true, kmsLicense: "value" });
		});
	});
});
