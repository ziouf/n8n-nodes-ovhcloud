...
import { description, execute } from './vmBackupJobEnable.operation';
...
describe('vmBackupJobEnable.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct endpoint', async () => {
			mockExecuteFunctions.getNodeParameter.mockImplementation((param: string): any => {
				...
				if (param === 'backupDays') return ['Monday', 'Friday'];
				return undefined;
			});
			...
			expect(client.httpPost).toHaveBeenCalledWith('/dedicatedCloud/pcc-123-456-789/datacenter/1/vm/1/backupJob/enable', { backupDays: ["Monday", "Friday"] });
		});
	});
});
