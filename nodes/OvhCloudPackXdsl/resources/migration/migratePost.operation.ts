...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			type: 'string',
			default: '',
			description: 'Product code, a unique identifier to designate the chosen offer',
			displayOptions,
		},
		{
			...
			type: 'string',
			default: '',
			description: 'Residence identifier, "_NA_" if no identifier is available',
			displayOptions,
		},
		{
			displayName: 'Stair',
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(
		`/pack/xdsl/${encodeURIComponent(packName)}/migration/migrate`,
		body,
	)) as IDataObject;
