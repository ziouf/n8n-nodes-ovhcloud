...
import { description, execute } from './offersPost.operation';
...
describe('offersPost.operation', () => {
	describe('description', () => {
		it('should return packName and buildingReference parameters', () => {
	...
	describe('execute', () => {
		...
		it('should get migration offers via POST with buildingReference', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/pack/xdsl/packabcd-ovh/migration/offers', {
				buildingReference: 'buildingRef123',
			});
			expect(result).toEqual([mockData]);
		});

		it('should send an empty body when buildingReference is not provided', async () => {
		...
		it('should encode special characters in packName', async () => {
			...
			await execute.call(mockExecuteFunctions);
			expect(client.httpPost).toHaveBeenCalledWith('/pack/xdsl/pack%20abcd/migration/offers', {});
		});
	});
});
