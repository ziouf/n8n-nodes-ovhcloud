...
import { description, execute } from './exchangeIndividualOptionsIsEmailAvailableGet.operation';
...
describe('exchangeIndividualOptionsIsEmailAvailableGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should check email availability via GET with email query parameter', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith(
				'/pack/xdsl/packabcd-ovh/exchangeIndividual/options/isEmailAvailable',
		...
		it('should encode special characters in packName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith(
				'/pack/xdsl/pack%20abcd/exchangeIndividual/options/isEmailAvailable',
				{ email: 'test@example.com' },
			);
		});
	});
