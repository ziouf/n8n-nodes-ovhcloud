...
/**
 * List the Exchange 2013 organization services of the pack.
 *
 * HTTP method: GET
 * Endpoint: /pack/xdsl/{packName}/exchangeOrganization/services
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	const client = new ApiClient(this);
	const packName = this.getNodeParameter('packName', _itemIndex ?? 0, '', { extractValue: true }) as string;
	const data = (await client.httpGet(
		`/pack/xdsl/${encodeURIComponent(packName)}/exchangeOrganization/services`,
	)) as unknown[];
