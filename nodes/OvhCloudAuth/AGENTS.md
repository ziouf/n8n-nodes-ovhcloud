...
- **API Spec**: https://eu.api.ovh.com/v1/auth.json
- **Base Path**: /auth
...
├── list.operation.ts      # GET /auth
├── get.operation.ts       # GET /auth/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getAuthServices'`)
