...
import { description, execute } from './diagnosticDetailGet.operation';
...
describe('diagnosticDetailGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/diagnostic/12345');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/diagnostic/98765');
		});
	});
});