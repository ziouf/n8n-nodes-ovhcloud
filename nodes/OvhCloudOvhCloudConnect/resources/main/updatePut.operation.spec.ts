...
describe('updatePut.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via PUT', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/ovhCloudConnect/svc-1', { description: 'sample-value' });
expect(result).toEqual([{ serviceName: 'svc-1', success: true }]);
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/ovhCloudConnect/svc%201', { description: 'alternate-value' });
		});
	});
});