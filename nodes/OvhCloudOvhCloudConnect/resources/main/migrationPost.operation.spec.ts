...
import { description, execute } from './migrationPost.operation';
...
describe('migrationPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/migration');
expect(result).toEqual([{ serviceName: 'svc-1', success: true }]);
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/migration');
		});
	});
});