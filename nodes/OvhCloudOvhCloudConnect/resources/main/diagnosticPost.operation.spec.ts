...
import { description, execute } from './diagnosticPost.operation';
...
describe('diagnosticPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/diagnostic', { function: 'capture', isSsh: true, sshKey: 'sample-value', sshPort: 12345 });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/diagnostic', { function: 'ping', isSsh: false, sshKey: 'alternate-value', sshPort: 98765 });
		});
	});
});