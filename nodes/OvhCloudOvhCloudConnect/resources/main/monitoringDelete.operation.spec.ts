...
import { description, execute } from './monitoringDelete.operation';
...
describe('monitoringDelete.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via DELETE', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/monitoring');
expect(result).toEqual([{ serviceName: 'svc-1', success: true }]);
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/monitoring');
		});
	});
});