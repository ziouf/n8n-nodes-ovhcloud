...
import { description, execute } from './popDatacenterExtraDelete.operation';
...
describe('popDatacenterExtraDelete.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via DELETE', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/config/pop/12345/datacenter/12345/extra/12345');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/config/pop/98765/datacenter/98765/extra/98765');
		});
	});
});