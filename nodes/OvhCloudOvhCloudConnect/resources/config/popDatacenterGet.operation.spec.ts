...
import { description, execute } from './popDatacenterGet.operation';
...
describe('popDatacenterGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/config/pop/12345/datacenter');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/config/pop/98765/datacenter');
		});
	});
});