...
/**
 * List the POP configurations linked to an OvhCloud Connect service.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(`/ovhCloudConnect/${encodeURIComponent(serviceName)}/config/pop`)) as unknown[];
	const items = data.map((item) =>
		typeof item === 'string' ? { popId: item } : (item as IDataObject),
	) as IDataObject[];
