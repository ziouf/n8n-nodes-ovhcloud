...
/**
 * List the extra configurations for a datacenter configuration.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(`/ovhCloudConnect/${encodeURIComponent(serviceName)}/config/pop/${encodeURIComponent(popId)}/datacenter/${encodeURIComponent(datacenterId)}/extra`)) as unknown[];
	const items = data.map((item) =>
		typeof item === 'string' ? { extraId: item } : (item as IDataObject),
	) as IDataObject[];
