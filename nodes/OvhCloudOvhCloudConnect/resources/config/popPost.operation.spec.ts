...
import { description, execute } from './popPost.operation';
...
describe('popPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/ovhCloudConnect/svc-1/config/pop', { datacenter: 'sample-value', region: 'sample-value', plan: 'sample-value' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/ovhCloudConnect/svc%201/config/pop', { datacenter: 'alternate-value', region: 'alternate-value', plan: 'alternate-value' });
		});
	});
});