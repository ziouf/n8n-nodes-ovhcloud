...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			type: 'boolean',
			default: false,
			description: 'Whether to include detailed information for each service',
			displayOptions,
		}
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const withDetails = this.getNodeParameter('svcWithDetails', 0, { extractValue: true });
	...
	if (!withDetails) {
		return serviceIds.map((serviceId) => ({ json: { serviceId } }));
	}
