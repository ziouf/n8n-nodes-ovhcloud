...
describe('logUrlPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc-1/log/url', { kind: 'sample-value' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc%201/log/url', { kind: 'alternate-value' });
		});
	});
});