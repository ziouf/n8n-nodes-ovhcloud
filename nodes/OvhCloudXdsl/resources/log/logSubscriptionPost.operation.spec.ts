...
describe('logSubscriptionPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc-1/log/subscription', { kind: 'sample-value', streamId: 'sample-value' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc%201/log/subscription', { kind: 'alternate-value', streamId: 'alternate-value' });
		});
	});
});