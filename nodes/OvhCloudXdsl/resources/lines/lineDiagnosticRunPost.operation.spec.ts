...
import { description, execute } from './lineDiagnosticRunPost.operation';
...
describe('lineDiagnosticRunPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc-1/lines/sample-value/diagnostic/run', { lineNumber: 'sample-value', period: 'day' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc%201/lines/alternate-value/diagnostic/run', { lineNumber: 'alternate-value', period: 'month' });
		});
	});
});