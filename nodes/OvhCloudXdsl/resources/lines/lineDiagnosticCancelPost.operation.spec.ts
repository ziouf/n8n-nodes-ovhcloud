...
import { description, execute } from './lineDiagnosticCancelPost.operation';
...
describe('lineDiagnosticCancelPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc-1/lines/sample-value/diagnostic/cancel');
expect(result).toEqual([{ serviceName: 'svc-1', success: true }]);
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc%201/lines/alternate-value/diagnostic/cancel');
		});
	});
});