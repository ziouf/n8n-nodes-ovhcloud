...
import { description, execute } from './linesStatisticsGet.operation';
...
describe('linesStatisticsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/lines/sample-value/statistics', { period: 'day', type: 'attenuation' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/lines/alternate-value/statistics', { period: 'month', type: 'errorsPower' });
		});
	});
});