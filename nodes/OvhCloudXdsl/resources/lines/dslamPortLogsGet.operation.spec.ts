...
import { description, execute } from './dslamPortLogsGet.operation';
...
describe('dslamPortLogsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/lines/sample-value/dslamPort/logs', { limit: 12345 });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/lines/alternate-value/dslamPort/logs', { limit: 98765 });
		});
	});
});