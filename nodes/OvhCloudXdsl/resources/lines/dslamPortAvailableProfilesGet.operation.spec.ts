...
import { description, execute } from './dslamPortAvailableProfilesGet.operation';
...
describe('dslamPortAvailableProfilesGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/lines/sample-value/dslamPort/availableProfiles');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/lines/alternate-value/dslamPort/availableProfiles');
		});
	});
});