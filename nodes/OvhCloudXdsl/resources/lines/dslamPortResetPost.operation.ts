...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		destructiveActionNotice('This will reset the lines. This action is irreversible.', displayOptions),
		{
...
/**
 * Reset a specific DSLAM port of an xDSL service.
 *
 * HTTP method: POST
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(`/xdsl/${encodeURIComponent(serviceName)}/lines/${encodeURIComponent(number)}/dslamPort/reset`)) as IDataObject;
