...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'IP address of the anti-spam',
			displayOptions,
		},
	];
}
...
/**
 * List the evidences stored on the PCS for a specific IP address.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(`/xdsl/${encodeURIComponent(serviceName)}/antiSpams/${encodeURIComponent(ip)}/evidences`)) as IDataObject;
