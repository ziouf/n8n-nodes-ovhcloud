...
import { description, execute } from './modemUpdatePut.operation';
...
describe('modemUpdatePut.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via PUT', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/xdsl/svc-1/modem', { brandName: 'sample-value', easyFirewallLevel: 'blockAll', ipv6Support: 'disabled' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPut).toHaveBeenCalledWith('/xdsl/svc%201/modem', { brandName: 'alternate-value', easyFirewallLevel: 'disabled', ipv6Support: 'enabled' });
		});
	});
});