...
import { description, execute } from './fiberEligibilitiesGet.operation';
...
describe('fiberEligibilitiesGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/fiberEligibilities', { status: 'active' });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/fiberEligibilities', { status: 'pending' });
		});
	});
});