...
import { description, execute } from './antiSpamsGet.operation';
...
describe('antiSpamsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/antiSpams');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/antiSpams');
		});
	});
});