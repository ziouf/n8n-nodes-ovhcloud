...
import { description, execute } from './fiberEligibilityDetailGet.operation';
...
describe('fiberEligibilityDetailGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/fiberEligibilities/12345');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/fiberEligibilities/98765');
		});
	});
});