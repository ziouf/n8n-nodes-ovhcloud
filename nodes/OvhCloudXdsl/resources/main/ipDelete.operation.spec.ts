...
import { description, execute } from './ipDelete.operation';
...
describe('ipDelete.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via DELETE', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/xdsl/svc-1/ips/sample-value');
expect(result).toEqual([{ serviceName: 'svc-1', success: true }]);
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/xdsl/svc%201/ips/alternate-value');
		});
	});
});