...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		{
			displayName: 'Fiber Eligibility ID',
			name: 'id',
			type: 'number',
			default: 0,
			required: true,
			description: 'ID of the fiber eligibility',
			displayOptions,
		},
	];
}
...
/**
 * Get the properties of a specific fiber eligibility of an xDSL service.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(`/xdsl/${encodeURIComponent(serviceName)}/fiberEligibilities/${encodeURIComponent(id)}`)) as IDataObject;
