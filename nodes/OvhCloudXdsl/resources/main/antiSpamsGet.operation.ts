...
/**
 * List the anti-spam IPs configured for an xDSL service.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(`/xdsl/${encodeURIComponent(serviceName)}/antiSpams`)) as unknown[];
	const items = data.map((item) =>
		typeof item === 'string' ? { ip: item } : (item as IDataObject),
	) as IDataObject[];
