...
import { description, execute } from './incidentsGet.operation';
...
describe('incidentsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/incidents', { creationDate: 'sample-value', endDate: 'sample-value' });
		...
		it('should omit empty optional parameters from the request', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/incidents', {});
		});
	});
});