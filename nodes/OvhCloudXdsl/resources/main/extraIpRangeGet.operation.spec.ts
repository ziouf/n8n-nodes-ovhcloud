...
import { description, execute } from './extraIpRangeGet.operation';
...
describe('extraIpRangeGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc-1/addressMove/extraIpRange');
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/xdsl/svc%201/addressMove/extraIpRange');
		});
	});
});