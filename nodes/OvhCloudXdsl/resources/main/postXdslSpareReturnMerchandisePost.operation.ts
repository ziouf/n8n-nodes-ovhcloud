...
/**
 * Return merchandise for a spare part
 *
 * HTTP method: POST
 * Endpoint: /xdsl/spare/{id}/returnMerchandise
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	const client = new ApiClient(this);
	const id = this.getNodeParameter('id', _itemIndex ?? 0) as string;

	const data = (await client.httpPost(`/xdsl/spare/${id}/returnMerchandise`, {})) as IDataObject;
