...
import { description, execute } from './mailSendingPost.operation';
...
describe('mailSendingPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc-1/mailSending', { enabled: true });
		...
		it('should encode special characters in path parameters', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/xdsl/svc%201/mailSending', { enabled: false });
		});
	});
});