...
/**
 * Create full access permission for an account in an Email Mxplan service.
 *
 * HTTP method: POST
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const body: IDataObject = {
		allowedAccountId: this.getNodeParameter('allowedAccountId', 0) as string,
	};
	const data = (await client.httpPost(`/email/mxplan/${service}/account/${email}/fullAccess`, {
		body,
	})) as IDataObject;
	return [{ json: data }];
}
