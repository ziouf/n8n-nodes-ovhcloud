...
import { description, execute } from './userUsernameDelete.operation';
...
describe('userUsernameDelete.operation', () => {
	...
	describe('execute', () => {
		...
		it('should remove a user via DELETE with encoded username', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/cluster/hadoop/cluster-12345/user/user1');
		...
		it('should encode special characters in serviceName and username', async () => {
			...
			expect(client.httpDelete).toHaveBeenCalledWith('/cluster/hadoop/cluster%201/user/user%201');
		});
	});
});
