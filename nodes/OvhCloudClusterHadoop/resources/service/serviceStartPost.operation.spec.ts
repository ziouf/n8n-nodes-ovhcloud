...
import { description, execute } from './serviceStartPost.operation';
...
describe('serviceStartPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should start a service via POST with service name', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/cluster/hadoop/cluster-12345/service/start', {
				service: 'Hive',
			});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/cluster/hadoop/cluster%201/service/start', {
				service: 'HDFS',
			});
		});
	});
