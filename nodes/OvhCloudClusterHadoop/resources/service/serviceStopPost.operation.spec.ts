...
import { description, execute } from './serviceStopPost.operation';
...
describe('serviceStopPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should stop a service via POST with service name', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/cluster/hadoop/cluster-12345/service/stop', {
				service: 'Hive',
			});
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/cluster/hadoop/cluster%201/service/stop', {
				service: 'HDFS',
			});
		});
	});
