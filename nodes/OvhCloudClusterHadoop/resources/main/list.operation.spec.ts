...
describe('list.operation', () => {
	...
	describe('execute', () => {
		...
		it('should list Hadoop clusters via GET', async () => {
			const mockData = ['cluster-12345', 'cluster-67890'];
			const client = new ApiClient(mockExecuteFunctions) as any;
			client.httpGet.mockResolvedValue(mockData);

			const result = await execute.call(mockExecuteFunctions);
			expect(client.httpGet).toHaveBeenCalledWith('/cluster/hadoop');
			expect(result).toEqual([{ serviceName: 'cluster-12345' }, { serviceName: 'cluster-67890' }]);
		});
