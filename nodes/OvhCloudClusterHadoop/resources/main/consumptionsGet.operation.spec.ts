...
import { description, execute } from './consumptionsGet.operation';
...
describe('consumptionsGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get cluster consumptions via GET', async () => {
			const mockData = { total: 42.5 };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/cluster/hadoop/cluster-12345/consumptions');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/cluster/hadoop/cluster%201/consumptions');
		});
	});
});
