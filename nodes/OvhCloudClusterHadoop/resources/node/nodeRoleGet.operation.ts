...
/**
 * List the roles (set of Hadoop services) of a node.
 *
 * HTTP method: GET
 * Endpoint: /cluster/hadoop/{serviceName}/node/{hostname}/role
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpGet(
		`/cluster/hadoop/${encodeURIComponent(serviceName)}/node/${encodeURIComponent(hostname)}/role`,
	)) as unknown[];
	const items = data.map((item) =>
		typeof item === 'string' ? { type: item } : (item as IDataObject),
	) as IDataObject[];
