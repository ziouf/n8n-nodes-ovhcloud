...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			options: ROLE_TYPES,
			required: true,
			description: 'Role name to start',
			displayOptions,
		},
	];
}
...
/**
 * Start a role on a node.
 *
 * HTTP method: POST
 * Endpoint: /cluster/hadoop/{serviceName}/node/{hostname}/role/{type}/start
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(
		`/cluster/hadoop/${encodeURIComponent(serviceName)}/node/${encodeURIComponent(hostname)}/role/${encodeURIComponent(type)}/start`,
	)) as IDataObject;
