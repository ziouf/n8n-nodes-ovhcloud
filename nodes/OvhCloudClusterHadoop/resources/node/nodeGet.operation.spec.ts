...
import { description, execute } from './nodeGet.operation';
...
describe('nodeGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get node properties via GET with encoded hostname', async () => {
			const mockData = { hostname: 'node-12345', status: 'active' };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/cluster/hadoop/cluster-12345/node/node-12345');
		...
		it('should encode special characters in serviceName and hostname', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/cluster/hadoop/cluster%201/node/node%201');
		});
	});
});
