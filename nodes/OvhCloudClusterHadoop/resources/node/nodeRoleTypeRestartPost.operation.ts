...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			options: ROLE_TYPES,
			required: true,
			description: 'Role name to restart',
			displayOptions,
		},
	];
}
...
/**
 * Restart a role on a node (this action will restart other dependant roles).
 *
 * HTTP method: POST
 * Endpoint: /cluster/hadoop/{serviceName}/node/{hostname}/role/{type}/restart
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(
		`/cluster/hadoop/${encodeURIComponent(serviceName)}/node/${encodeURIComponent(hostname)}/role/${encodeURIComponent(type)}/restart`,
	)) as IDataObject;
