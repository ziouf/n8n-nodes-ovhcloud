...
import { description, execute } from './nodeRoleTypeRestartPost.operation';
...
describe('nodeRoleTypeRestartPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should restart a node role via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster-12345/node/node-12345/role/data_node/restart',
			);
		...
		it('should encode special characters in serviceName and hostname', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster%201/node/node%201/role/name_node/restart',
			);
		});
	});
});
