...
import { description, execute } from './nodeRoleTypeGet.operation';
...
describe('nodeRoleTypeGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should get node role properties via GET with encoded type', async () => {
			const mockData = { type: 'data_node', status: 'started' };
			...
			expect(client.httpGet).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster-12345/node/node-12345/role/data_node',
			);
		...
		it('should encode special characters in serviceName and hostname', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster%201/node/node%201/role/name_node',
			);
		});
	});
});
