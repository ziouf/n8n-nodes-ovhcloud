...
/**
 * Stop a role on a node (this action will stop other dependant roles).
 *
 * HTTP method: POST
 * Endpoint: /cluster/hadoop/{serviceName}/node/{hostname}/role/{type}/stop
 */
export async function execute(this: IExecuteFunctions, _itemIndex?: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(
		`/cluster/hadoop/${encodeURIComponent(serviceName)}/node/${encodeURIComponent(hostname)}/role/${encodeURIComponent(type)}/stop`,
	)) as IDataObject;
