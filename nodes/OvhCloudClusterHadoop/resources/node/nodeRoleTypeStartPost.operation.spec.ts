...
import { description, execute } from './nodeRoleTypeStartPost.operation';
...
describe('nodeRoleTypeStartPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should start a node role via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster-12345/node/node-12345/role/data_node/start',
			);
		...
		it('should encode special characters in serviceName and hostname', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster%201/node/node%201/role/name_node/start',
			);
		});
	});
});
