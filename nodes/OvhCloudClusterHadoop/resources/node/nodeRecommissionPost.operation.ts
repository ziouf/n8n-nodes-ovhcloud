...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			description: 'Hostname of the node to recommission',
			displayOptions,
		},
	];
}
...
/**
 * Recommission a node and all the services on it.
 *
 * HTTP method: POST
 * Endpoint: /cluster/hadoop/{serviceName}/node/{hostname}/recommission
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(
		`/cluster/hadoop/${encodeURIComponent(serviceName)}/node/${encodeURIComponent(hostname)}/recommission`,
	)) as IDataObject;
