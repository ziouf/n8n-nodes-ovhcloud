...
import { description, execute } from './nodeDecommissionPost.operation';
...
describe('nodeDecommissionPost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should decommission a node via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster-12345/node/node-12345/decommission',
			);
		...
		it('should encode special characters in serviceName and hostname', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith(
				'/cluster/hadoop/cluster%201/node/node%201/decommission',
			);
		});
	});
});
