...
/**
 * @brief Create Backup FTP Access ACL operation for Dedicated Housing
 *
 * Creates a new ACL entry for the backup FTP of a Dedicated Housing service.
 *
 * HTTP method: POST
 * Endpoint: /dedicated/housing/{serviceName}/features/backupFTP/access
 * Body: ipBlock (required), cifs (required), nfs (required), ftp (optional)
 */
export function descriptionBackupFtpCreateAccess(
...
/**
 * Executes the Create Backup FTP Access ACL operation.
 *
 * @param this - n8n IExecuteFunctions context
 * @returns Array of execution results containing the created ACL
 */
export async function executeBackupFtpCreateAccess(
