...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			type: 'json',
			default: '',
			description: 'Request body (dbaas.logs.OutputGraylogStreamUpdate)',
			displayOptions,
		},
	];
}
...
/**
 * Executes the PUT outputGraylogStreamUpdatePut operation.
 *
 * HTTP method: PUT
...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPut(`/dbaas/logs/${encodeURIComponent(serviceName)}/output/graylog/stream/${encodeURIComponent(streamId)}`, body)) as IDataObject;
