...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			type: 'json',
			default: '',
			description: 'Request body (dbaas.logs.OutputOpenSearchAliasIndexCreation)',
			displayOptions,
		},
	];
}
...
/**
 * Executes the POST outputOpenSearchAliasIndexCreatePost operation.
 *
 * HTTP method: POST
...
export async function execute(this: IExecuteFunctions, _itemIndex: number): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(`/dbaas/logs/${encodeURIComponent(serviceName)}/output/opensearch/alias/${encodeURIComponent(aliasId)}/index`, body)) as IDataObject;
