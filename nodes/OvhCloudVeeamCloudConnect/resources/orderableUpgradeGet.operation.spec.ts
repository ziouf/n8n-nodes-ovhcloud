...
import { description, execute } from './orderableUpgradeGet.operation';
...
describe('orderableUpgradeGet.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via GET', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/veeamCloudConnect/svc-1/orderableUpgrade');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpGet).toHaveBeenCalledWith('/veeamCloudConnect/svc%201/orderableUpgrade');
		});
	});
});
