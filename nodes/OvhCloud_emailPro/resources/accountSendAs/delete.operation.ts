...
/**
 * Description for Delete Account Send As operation.
 */
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'Account ID to revoke send-as from',
			displayOptions,
		},
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpDelete(
		`/email/pro/${serviceName}/account/${email}/sendAs/${allowedAccountId}`,
	)) as IDataObject;
	return [{ json: data }];
}
