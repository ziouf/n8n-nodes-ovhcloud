...
- **API Spec**: https://eu.api.ovh.com/v1/newAccount.json
- **Base Path**: /newAccount
...
├── list.operation.ts      # GET /newAccount
├── get.operation.ts       # GET /newAccount/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getNewAccountServices'`)
