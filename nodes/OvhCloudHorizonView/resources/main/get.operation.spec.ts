...
describe('get.operation', () => {
	describe('description', () => {
		...
		it('should have list and name modes for the service locator', () => {
			...
			expect(nameMode.placeholder).toBe('service1');
		});
	});
	...
	describe('execute', () => {
		...
		it('should get Horizon View service via GET', async () => {
			const mockData = { datacenterId: 'dc1' };
			...
			expect(client.httpGet).toHaveBeenCalledWith('/horizonView/service1');
		...
		it('should encode special characters in serviceName', async () => {
			...
			mockExecuteFunctions.getNodeParameter.mockImplementation(
				(param: string, _idx?: number, def?: any, opts?: any): any => {
					if (opts?.extractValue && param === 'serviceName') return 'service 1';
					switch (param) {
						case 'serviceName':
							return 'service 1';
						default:
			...
			expect(client.httpGet).toHaveBeenCalledWith('/horizonView/service%201');
		});
	});
});
