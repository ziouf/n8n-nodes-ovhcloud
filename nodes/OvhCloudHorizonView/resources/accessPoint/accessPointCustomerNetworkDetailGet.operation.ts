...
/**
 * Get properties of a customer network.
 *
 * HTTP method: GET
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const customerNetworkId = this.getNodeParameter('customerNetworkId', 0) as number;
	const data = (await client.httpGet(
		`/horizonView/${encodeURIComponent(serviceName)}/accessPoint/${encodeURIComponent(accessPointId)}/customerNetwork/${customerNetworkId}`,
	)) as IDataObject;
