...
/**
 * Executes the List AI Solutions Notebook editors operation.
 *
 * HTTP method: GET
 * Endpoint: /cloud/project/{serviceName}/ai/capabilities/region/{region}/notebook/editor
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const serviceName = this.getNodeParameter('serviceName', 0) as string;
	const region = this.getNodeParameter('region', 0) as string;
	const compatibleWithFramework = this.getNodeParameter('compatibleWithFramework', 0) as string;

	const qs: Record<string, string> = {};
	if (compatibleWithFramework) qs.compatibleWithFramework = compatibleWithFramework;

	const client = new ApiClient(this);
	const data = (await client.httpGet('cloud/project' + serviceName + '/ai/capabilities/region/' + region + '/notebook/editor', qs)) as IDataObject;
