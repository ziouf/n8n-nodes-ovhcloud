import type {
	IDataObject,
	IExecuteFunctions,
	IDisplayOptions,
	INodeExecutionData,
	INodeProperties,
} from 'n8n-workflow';
import { ApiClient } from '../../../shared/transport/ApiClient';

export function description(displayOptions: IDisplayOptions = {} as IDisplayOptions): INodeProperties[] {
	return [
		{
			displayName: 'Service Name',
			name: 'serviceName',
			type: 'resourceLocator',
			default: { mode: 'list', value: '' },
			required: true,
			description: 'The OVHcloud service name',
			modes: [
				{
					displayName: 'From List',
					name: 'list',
					type: 'list',
					typeOptions: { searchListMethod: 'getPublicCloudProjects' },
				},
				{
					displayName: 'By Name',
					name: 'name',
					type: 'string',
				},
			],
			displayOptions,
		},
		{
			displayName: 'Job ID',
			name: 'jobId',
			type: 'string',
			default: '',
			required: true,
			description: 'The jobId parameter',
			displayOptions,
		},
	];
}

/**
 * Executes the Starts a manual data synchronization on an AI Solutions job operation.
 *
 * HTTP method: POST
 * Endpoint: /cloud/project/{serviceName}/ai/job/{jobId}/datasync
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const serviceName = this.getNodeParameter('serviceName', 0) as string;
	const jobId = this.getNodeParameter('jobId', 0) as string;

	const body = this.getNodeParameter("body", 0) as IDataObject;

	const client = new ApiClient(this);
	const data = (await client.httpPost('cloud/project' + serviceName + '/ai/job/' + jobId + '/datasync', body)) as IDataObject;

	return this.helpers.returnJsonArray([data]);
}
