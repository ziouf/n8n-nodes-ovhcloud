...
- **API Spec**: https://eu.api.ovh.com/v1/secret.json
- **Base Path**: /secret
...
├── list.operation.ts      # GET /secret
├── get.operation.ts       # GET /secret/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getSecretServices'`)
