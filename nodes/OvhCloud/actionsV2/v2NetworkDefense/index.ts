...
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	const operationProperties: INodeProperties[] = [
		{
			...
			name: 'v2NetworkDefenseOperation',
			type: 'options',
	...
	return [
		...
		...descriptionList({
			...displayOptions,
			show: { ...displayOptions?.show, v2NetworkDefenseOperation: ['list'] },
		}),
		...descriptionGet({
			...displayOptions,
			show: { ...displayOptions?.show, v2NetworkDefenseOperation: ['get'] },
		}),
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const operation = this.getNodeParameter('v2NetworkDefenseOperation', 0, { extractValue: true });
	...
	throw new Error(`Unsupported operation "${operation}" for resource "v2NetworkDefense"`);
}
