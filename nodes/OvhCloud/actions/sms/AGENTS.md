...
- **API Spec**: https://eu.api.ovh.com/v1/sms.json
- **Base Path**: /sms
...
├── list.operation.ts      # GET /sms
├── get.operation.ts       # GET /sms/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getSmsServices'`)
