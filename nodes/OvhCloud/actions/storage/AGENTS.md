...
- **API Spec**: https://eu.api.ovh.com/v1/storage.json
- **Base Path**: /storage
...
├── list.operation.ts      # GET /storage
├── get.operation.ts       # GET /storage/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getStorageServices'`)
