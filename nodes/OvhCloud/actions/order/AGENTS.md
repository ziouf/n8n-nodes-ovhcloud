...
- **API Spec**: https://eu.api.ovh.com/v1/order.json
- **Base Path**: /order
...
├── list.operation.ts      # GET /order
├── get.operation.ts       # GET /order/{serviceName}
└── resources/
...
6. **serviceName**: `resourceLocator` with modes `str` and `list` (searchListMethod: `'getOrderServices'`)
