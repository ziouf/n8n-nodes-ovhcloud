...
/**
 * Description for Create Account Send On Behalf To operation.
 */
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'Account ID to allow sending on behalf to',
			displayOptions,
		},
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpPost(
		`/email/pro/${serviceName}/account/${email}/sendOnBehalfTo`,
		body,
	)) as IDataObject;
	return [{ json: data }];
}
