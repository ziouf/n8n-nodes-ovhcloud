...
/**
 * Description for Delete Account Send On Behalf To operation.
 */
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'Account ID to revoke send-on-behalf-to from',
			displayOptions,
		},
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpDelete(
		`/email/pro/${serviceName}/account/${email}/sendOnBehalfTo/${allowedAccountId}`,
	)) as IDataObject;
	return [{ json: data }];
}
