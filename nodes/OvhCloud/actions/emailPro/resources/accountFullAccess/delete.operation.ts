...
/**
 * Description for Delete Account Full Access operation.
 */
export function description(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: '',
			required: true,
			description: 'Account ID to revoke full access from',
			displayOptions,
		},
	];
}
...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	const data = (await client.httpDelete(
		`/email/pro/${serviceName}/account/${email}/fullAccess/${allowedAccountId}`,
	)) as IDataObject;
	return [{ json: data }];
}
