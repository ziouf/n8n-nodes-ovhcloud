...
/**
 * @brief Delete CDN Cache Rule operation
 *
 * Removes a cache rule from a domain on a CDN Dedicated service.
 *
 * HTTP method: DELETE
 * Endpoint: /cdn/dedicated/{serviceName}/domains/{domain}/cacheRules/{cacheRuleId}
 */
export function descriptionCacheRulesDelete(displayOptions: IDisplayOptions): INodeProperties[] {
	return [
		{
		...
		},
		{
			...
			name: 'cacheRuleId',
			description: 'The ID of the cache rule to delete',
			type: 'string',
			required: true,
			default: '',
			displayOptions,
...
/**
 * Executes the Delete CDN Cache Rule operation.
 *
 * @param this - n8n IExecuteFunctions context
 * @returns Array of execution results
 */
export async function executeCacheRulesDelete(
