...
/**
 * @brief Get Domain Auth Info operation
 *
 * Retrieves authentication information for a domain.
 *
 * HTTP method: GET
...
/**
 * Executes the Get Domain Auth Info operation.
 */
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const client = new ApiClient(this);
	const serviceName = this.getNodeParameter('serviceName', 0, { extractValue: true }) as string;

	const data = (await client.httpGet(`/domain/${serviceName}/authInfo`)) as IDataObject;
	return [{ json: data }];
}
