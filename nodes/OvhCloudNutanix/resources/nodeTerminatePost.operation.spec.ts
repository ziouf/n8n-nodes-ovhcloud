...
import { description, execute } from './nodeTerminatePost.operation';
...
describe('nodeTerminatePost.operation', () => {
	...
	describe('execute', () => {
		...
		it('should call the correct API endpoint via POST', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/nutanix/svc-1/nodes/server-v/terminate');
		...
		it('should encode special characters in serviceName', async () => {
			...
			expect(client.httpPost).toHaveBeenCalledWith('/nutanix/svc%201/nodes/server-v/terminate');
		});
	});
});
